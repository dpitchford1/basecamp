...
if ( ! class_exists( 'Basecamp_Plugin_Install' ) ) :
	/**
	 * The Basecamp plugin install class
	 */
	class Basecamp_Plugin_Install {
		...
		public function plugin_install_scripts( $hook_suffix ) {
			...
			// wp_enqueue_script( 'basecamp-plugin-install', get_template_directory_uri() . '/inc/admin/assets/js/plugin-install' . $suffix . '.js', array( 'jquery', 'updates' ), $basecamp_version, 'all' );

			// wp_enqueue_style( 'basecamp-plugin-install', get_template_directory_uri() . '/inc/admin/assets/css/plugin-install.css', array(), $basecamp_version, 'all' );
		}
		...
		public static function install_plugin_button( $plugin_slug, $plugin_file, $plugin_name, $classes = array(), $activated = '', $activate = '', $install = '' ) {
			if ( current_user_can( 'install_plugins' ) && current_user_can( 'activate_plugins' ) ) {
				if ( is_plugin_active( $plugin_slug . '/' . $plugin_file ) ) {
					...
					$button = array(
						'message' => esc_attr__( 'Activated', 'basecamp' ),
						'url'     => '#',
						'classes' => array( 'basecamp-button', 'disabled' ),
					);
					...
					$button = array(
						'message' => esc_attr__( 'Activate', 'basecamp' ),
						'url'     => $url,
					...
					$button = array(
						'message' => esc_attr__( 'Install now', 'basecamp' ),
						'url'     => $url,
				...
				</span> <?php echo /* translators: conjunction of two alternative options user can choose (in missing plugin admin notice). Example: "Activate WooCommerce or learn more" */ esc_html__( 'or', 'basecamp' ); ?>
				<a href="https://wordpress.org/plugins/<?php echo esc_attr( $plugin_slug ); ?>" target="_blank"><?php esc_html_e( 'learn more', 'basecamp' ); ?></a>
				<?php
			}
		}
...
return new Basecamp_Plugin_Install();
